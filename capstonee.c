...
User *login()
{
    ...
    while (fread(&user, sizeof(User), 1, file))
    {
        ...
        if (strcmp(user.username, username) == 0 && strcmp(decrypted, password) == 0)
        {
            fclose(file);
            return &user;
        }
...
void viewDoctors()
{
    ...
    while (fread(&d, sizeof(Doctor), 1, file))
    {
        printf("%-5d %-20s %-20s %-15s $%-9.2f $%-9.2f\n",
               d.id, d.name, d.specialization, d.availableTime, d.consultancyFee, d.profit);
    }
    fclose(file);
}
void deleteDoctor()
{
    ...
    while (fread(&d, sizeof(Doctor), 1, file))
    {
        ...
        {
            fwrite(&d, sizeof(Doctor), 1, tempFile);
        }
    }
...
void viewAppointments()
{
    ...
    while (fread(&apt, sizeof(Appointment), 1, file))
    {
        printf("%-5d %-8d %-8d %-20s %-10s\n",
               apt.id, apt.patientId, apt.doctorId, apt.date, apt.status);
    }
    fclose(file);
}
...
void viewBills()
{
    ...
    while (fread(&bill, sizeof(Bill), 1, file))
    {
        printf("%-5d %-8d %-8d $%-9.2f %s\n",
               bill.id, bill.patientId, bill.doctorId, bill.amount,
               bill.isPaid ? "Yes" : "No");
    }
    fclose(file);
}
...
void adminMenu()
{
    ...
    do
    {
        ...
        case 4:
            ...
            break;
        case 5:
            deleteDoctor(); // Call the deleteDoctor function
            break;
        case 6:
            viewAppointments();
            break;
...
void doctorMenu(int doctorId)
{
    ...
    do
    {
        ...
        case 3:
        {
            printf("log out successfully as doctor. thank you\n \n");
            return;
        }
        default:
            printf("Invalid choice.\n");
...
void patientMenu(int patientId)
{
    ...
    do
    {
        ...
        case 3:
            ...
            break;
        case 4:
            printf("log out susccessfully as patient");
            return;
        default:
            printf("Invalid choice.\n");
        }
